...
function CustomerOrderItem({order , CustomerInformation, num, setTrigger}) {
  ...
  useEffect(() => {
    CustomerInformation?.id ? get_order_type_data('/measurement-types/' + order?.measurement_type) : dataSeter([])
  ...
  return (
    <div className="new-container">
      ...
      {CustomerOrders?.results.map((order, num) => (
        <CustomerOrderItem key={order.id} order={order} num={num} CustomerInformation={CustomerInformation} setTrigger={setTrigger}/>
      ))}
    </div>
  );
}
